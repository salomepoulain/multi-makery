...
# --- validation ---
STATION_NAME="$1"
[ -z "$STATION_NAME" ] && SAY "Usage: bake first <station_name>"
...
TMP_DIR=$(mktemp -d)
git clone --depth 1 --filter=blob:none --sparse "$STATIONS_REPO" "$TMP_DIR" > /dev/null 2>&1 || SAY "Failed to contact Registry."
...
if [ ! -d "stations/$STATION_NAME" ]; then
    cd - > /dev/null || exit
    rm -rf "$TMP_DIR"
    SAY "The '$STATION_NAME' station doesn't exist in the Registry."
fi
...
if [ -f "$STATION_DIR/cook/contract/.prerequisite" ]; then
    ...
    while IFS= read -r dep || [ -n "$dep" ]; do
        ...
        if ! command -v "$dep" &> /dev/null; then
            SAY "✗ Missing: $dep"
            rm -rf "$STATION_DIR"
            SAY "Missing essential tools. Station setup aborted."
        else
            SAY "✓ Found: $dep"
        fi
